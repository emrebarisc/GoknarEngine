...
//  [X] Renderer: User textures_ binding. Use 'GLuint' OpenGL textures_ identifier as void*/ImTextureID. Read the FAQ about ImTextureID in imgui.cpp.
...
#ifdef GL_SAMPLER_BINDING
    glBindSampler(0, 0); // We use combined textures_/sampler state. Applications using GL 3.3 may set that otherwise.
#endif
...
bool ImGui_CreateFontsTexture()
{
    ...
    int width, height;
    io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);   // Load as RGBA 32-bits (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing shaders. If your ImTextureId represent a higher-level concept than just a GL textures_ id, consider calling GetTexDataAsAlpha8() instead to save on GPU memory.

    // Upload textures_ to graphics system
    GLint last_texture;
