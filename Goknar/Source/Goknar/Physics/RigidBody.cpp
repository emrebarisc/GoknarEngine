 ...
 */
static inline void _transformInertiaTensor(Matrix3x3& iitWorld, const Quaternion& q, const Matrix3x3& iitBody, const Matrix& rotmat)
{
    ...
    iitWorld.m[0] = t4 * rotmat[0] + t9 * rotmat[1] + t14 * rotmat[2]; 
    iitWorld.m[1] = t4 * rotmat[4] + t9 * rotmat[5] + t14 * rotmat[6]; 
    iitWorld.m[2] = t4 * rotmat[8] + t9 * rotmat[9] + t14 * rotmat[10]; 
    iitWorld.m[3] = t28 * rotmat[0] + t33 * rotmat[1] + t38 * rotmat[2]; 
    iitWorld.m[4] = t28 * rotmat[4] + t33 * rotmat[5] + t38 * rotmat[6]; 
    iitWorld.m[5] = t28 * rotmat[8] + t33 * rotmat[9] + t38 * rotmat[10]; 
    iitWorld.m[6] = t52 * rotmat[0] + t57 * rotmat[1] + t62 * rotmat[2]; 
    iitWorld.m[7] = t52 * rotmat[4] + t57 * rotmat[5] + t62 * rotmat[6]; 
...
{
    ObjectBase::SetWorldRotation(rotation);
