...
public:
	...
	MulticastDelegate& operator +=(const MulticastDelegate& another) {
		for (auto& item : another.invocationList) // clone, not copy; flattens hierarchy:
			this->invocationList.push_back(new typename DelegateBase<RET(PARAMS...)>::InvocationElement(item->object, item->stub));
		return *this;
	} //operator +=
	...
	MulticastDelegate& operator +=(const Delegate<RET(PARAMS...)>& another) {
		...
		this->invocationList.push_back(new typename DelegateBase<RET(PARAMS...)>::InvocationElement(another.invocation.object, another.invocation.stub));
		return *this;
	} //operator +=
...
private:
	...
	std::list<typename DelegateBase<RET(PARAMS...)>::InvocationElement *> invocationList;
