...
#define __GOKNARASSERT_H__
	...
	#include "Log.h"
		...
		#define GOKNAR_CORE_CHECK(condition, ...) { if(UNLIKELY(!(condition))){ GOKNAR_CORE_ERROR("Assertion Failed: {0}", TUPLE_TAIL(__VA_ARGS__)); DEBUG_BREAK(); } }
	...
	#else
		#define GOKNAR_CHECK(condition, ...) { if(UNLIKELY(!(condition))){ GOKNAR_ERROR("\n{}:{} Assertion Failed: {}", __FILE__, __LINE__ VA_ARGS(__VA_ARGS__)); DEBUG_BREAK(); } }
