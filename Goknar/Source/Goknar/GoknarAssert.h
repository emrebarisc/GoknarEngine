...
#define __GOKNARASSERT_H__
	...
	#include "Log.h"
		...
		#define GOKNAR_CORE_CHECK(condition, ...) { if(UNLIKELY(!(condition))){ GOKNAR_ERROR("Assertion Failed: {0}", TUPLE_TAIL(__VA_ARGS__)); DEBUG_BREAK(); } }
		...
		#define GOKNAR_CORE_CHECK(condition, ...) { if(UNLIKELY(!(condition))){ GOKNAR_ERROR("Assertion Failed: {0}", ##__VA_ARGS__); DEBUG_BREAK(); } }
