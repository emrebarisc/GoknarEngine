...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    // We specify a default position_/size in case there's no data in the .ini file. Typically this isn't required! We only do it to make the Demo applications a little more welcoming.
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Trees"))
    {
        ...
        if (ImGui::TreeNode("Advanced, with Selectable nodes"))
        {
            ...
            if (node_clicked != -1)
            {
                // Tick selection state. Process outside of tree loop to avoid visual inconsistencies during the clicking-frame.
    ...
    if (ImGui::TreeNode("Images"))
    {
        ...
        // Here we are grabbing the font textures_ because that's the only one we have access to inside the demo code.
        // Remember that ImTextureID is just storage for whatever you want it to be, it is essentially a value that will be passed to the render function inside the ImDrawCmd structure.
        // If you use one of the default imgui_impl_XXXX.cpp renderer, they all have comments at the top of their file to specify what they expect to be stored in ImTextureID.
        // (for example, the imgui_impl_dx11.cpp renderer expect a 'ID3D11ShaderResourceView*' pointer. The imgui_impl_glfw_gl3.cpp renderer expect a GLuint OpenGL textures_ identifier etc.)
    ...
    if (ImGui::TreeNode("Color/Picker Widgets"))
    {
        ...
        if (ImGui::BeginPopup("mypicker"))
        {
            ...
            ImGui::BeginGroup(); // Lock X position_
            ImGui::Text("Current");
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Child windows"))
    {
        ...
        // - Using SetCursorPos() to position_ the child window (because the child window is an item from the POV of the parent window)
        //   You can also call SetNextWindowPos() to position_ the child window. The parent window will effectively layout from this position_.
    ...
    if (ImGui::TreeNode("Basic Horizontal Layout"))
    {
        ...
        // Aligned to arbitrary position_. Easy/cheap column.
        ImGui::Text("Aligned");
        ...
        for (int n = 0; n < buttons_count; n++)
        {
            ...
            float next_button_x2 = last_button_x2 + style.ItemSpacing.x + button_sz.x; // Expected position_ if next button was on same line
    ...
    if (ImGui::TreeNode("Horizontal Scrolling"))
    {
        ...
        for (int line = 0; line < lines; line++)
        {
            ...
            // manipulating the cursor position_ yourself, aka using SetCursorPos/SetCursorScreenPos to position_ the widgets yourself. You may also want to use the lower-level ImDrawList API)
...
static void ShowDemoWindowPopups()
{
    ...
    if (ImGui::TreeNode("Modals"))
    {
        ...
        if (ImGui::BeginPopupModal("Delete?", NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            //ImGui::Combo("Combo", &dummy_i, "Destroy\0Destroy harder\0");
        ...
        case ImGuiInputTextFlags_CallbackCompletion:
            {
                ...
                {
                    // Single match. Destroy the beginning of the word and replace it entirely so we've got nice casing
        ...
        case ImGuiInputTextFlags_CallbackHistory:
            {
                ...
                // A better implementation would preserve the data on the current input line along with cursor position_.
