...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        // Scene (we don't hide text after ## in this end-user function)
...
{
    bool need_backup = (GImGui->CurrentWindow->DC.TextWrapPos < 0.0f);  // Keep existing wrap position_ if one is already set
    if (need_backup)
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    // Handle input right away. None of the code of Begin() is relying on scrolling position_ before calling Scrollbar().
    bool held = false;
    bool hovered = false;
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // Click position_ in scrollbar normalized space (0.0f->1.0f)
        ...
        // Apply scroll
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of SizeContents and before setting up our starting position_
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Default to using textures_ ID as ID. User can still push string/integer prefixes.
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Peak into expected window size so we can position_ it
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    // Avoid altering values and clamping when we are _already_ past the limits and heading in the same direction_, so e.g. if range is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (set_new_value)
        {
            ...
            else
            {
                ...
                {
                    // For integer values we want the clicking position_ to match the grab box so we round above
    ...
    // Output grab position_ so it can be displayed by the caller
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        // Preserve cursor position_ and undo/redo stack if we come back to same widget
        // FIXME: For non-readonly widgets we might be able to require that TextAIsValid && TextA == buf ? (untested) and discard undo stack if user buffer has changed.
        const bool recycle_state = (state->ID == id);
        if (recycle_state)
        {
            // Recycle existing cursor/selection/undo stack but clamp position_
            // Note a single mouse click will override the cursor/position_ immediately by calling stb_textedit_click handler.
            state->CursorClamp();
        }
        else
        {
    ...
    // Scene text. We currently only render selection when the widget is active or while scrolling.
    ...
    if (render_cursor || render_selection)
    {
        ...
        // Scene text (with cursor and selection)
        // This is going to be messy. We need to:
        // - Display the text (this alone can be more easily clipped)
        // - Handle scrolling, highlight selection, display cursor (those all requires some form of 1d->2d cursor position_ calculation)
...

        {
            ...
            // Calculate 2d position_ by finding the beginning of the line and measuring distance
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                {
                    //p_ = (const ImWchar*)wmemchr((const wchar_t*)p_, '\n', text_selected_end - p_);  // FIXME-OPT: Could use this when wchar_t are 16-bits
                    //p_ = p_ ? p_ + 1 : text_selected_end;
        ...
        if (render_cursor)
        {
            ...
            // Notify OS of text input position_ for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
    ...
    {
        // Scene text only (no selection, no cursor)
        if (is_multiline)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        // Scene Cursor + preview on Hue Wheel
        float cos_hue_angle = ImCos(H * 2.0f * IM_PI);
        float sin_hue_angle = ImSin(H * 2.0f * IM_PI);
        ...
        // Scene SV triangle (rotated according to hue)
    ...
    // Scene cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
...
bool ImGui::BeginMenuBar()
{
    ...
    BeginGroup(); // Backup position_ on layer 0
    PushID("##menubar");
...
void ImGui::EndMenuBar()
{
    ...
    PopClipRect();
    PopID();
    window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->MenuBarRect().Min.x; // Save horizontal position_ so next append can reuse it. This is kinda equivalent to a per-layer CursorPos.
    window->DC.GroupStack.back().AdvanceCursor = false;
    EndGroup(); // Restore position_ on layer 0
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // The reference position_ stored in popup_pos will be used by Begin() to find a suitable position_ for the child menu,
    // However the final position_ is going to be different! It is choosen by FindBestWindowPosForPopup().
    // e.g. Menus tend to overlap each other horizontally to amplify relative Z-ordering.
    ImVec2 popup_pos, pos = window->DC.CursorPos;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Menu inside an horizontal menu bar
        // Selectable extend their highlight by half ItemSpacing in each direction_.
        // For ChildMenu, the popup position_ will be overwritten by the call to FindBestWindowPosForPopup() in Begin()
...
void ImGui::EndMenu()
{
    ...
    // However, it means that with the current code, a BeginMenu() from outside another menu or a menu-bar won't be closable with the Left direction_.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // Backup current layout position_
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a textures_), so need to add an extra draw call (temporary in the case of vertical animation)
    ...
    // Scene tab label, process close button
    ...
    // Restore main window position_ so user can draw there
    if (want_clip_rect)
        PopClipRect();
    window->DC.CursorPos = backup_main_cursor_pos;
...
// Scene text label (with custom clipping) + Unsaved Document marker + Close Button logic
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ...
    // Scene text label (with clipping + alpha gradient) + unsaved marker
