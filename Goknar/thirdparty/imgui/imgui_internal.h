...
struct ImDrawDataBuilder;           // Helper to build a ImDrawData instance_
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    CursorFollow;           // set when we want scrolling to follow the current cursor position_ (not always!)
...
struct ImGuiPopupRef
{
    ...
    ImVec2              OpenPopupPos;   // Set on OpenPopup(), preferred popup position_ (typically == OpenMousePos when using mouse)
    ImVec2              OpenMousePos;   // Set on OpenPopup(), copy of mouse position_ at the time of opening popup
};
...
struct ImGuiContext
{
    ...
    bool                    NavMousePosDirty;                   // When set we will update mouse position_ if (io.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos) if set (NB: this not enabled by default)
    ...
    ImGuiDir                NavMoveDir, NavMoveDirLast;         // Direction of the move request (left/right/up/down), direction_ of the previous move request
    ...
    // Scene
    ImDrawData              DrawData;                           // Main ImDrawData instance_ to pass render information to the user
    ...
    // Platform support
    ImVec2                  PlatformImePos;                     // Cursor position_ request & last passed to the OS Input Method Editor
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CursorStartPos;         // Initial position_ in client area with padding
    ...
    ImVec1                  Indent;                 // Indentation / start position_ from left of window (increased by TreePush/TreePop, etc.)
    ImVec1                  GroupOffset;
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  Scroll;
    ImVec2                  ScrollTarget;                       // target scroll position_. stored as cursor position_ with scrolling canceled out, so the highest point is always 0.0f. (FLT_MAX for no change)
    ImVec2                  ScrollTargetCenterRatio;            // 0.0f = scroll so that target position_ is at top, 0.5f = scroll so that target position_ is centered
    ...
    ImVec2                  SetWindowPosVal;                    // store window position_ when using a non-zero Pivot (position_ set needs to be processed when we know the window size)
    ...
    ImRect                  ContentsRegionRect;                 // FIXME: This is currently confusing/misleading. Maximum visible content position_ ~~ Pos + (SizeContentsExplicit ? SizeContentsExplicit : Size - ScrollbarSizes) - CursorStartPos, per axis
...
namespace ImGui
{
    ...
    // - You are calling ImGui functions after ImGui::EndFrame()/ImGui::Scene() and before the next ImGui::NewFrame(), which is also illegal.
    ...
    // NB: All position_ are in absolute pixels coordinates (we are never using window coordinates internally)
    ...
    // Scene helpers (those functions don't access any ImGui state!)
