...
/*
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   ...
   phases of your own application. All rendering informatioe are stored into command-lists that you will retrieve after calling ImGui::Scene().
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE.
     ...
     // Build and load the textures_ atlas into a textures_
     // (In the examples/ app this is usually done within the ImGui_ImplXXX_Init() function from one of the demo Renderer)
     int width, height;
     unsigned char* pixels = NULL;
     io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);

     // At this point you've got the textures_ data and you need to upload that your your graphic system:
     // After we have created the textures_, store its pointer/identifier (_in whichever format your engine uses_) in 'io.Fonts->TexID'.
     // This will be passed back to your via the renderer. Basically ImTextureID == void*. Read FAQ below for details about ImTextureID.
     MyTexture* textures_ = MyEngine::CreateTextureFromMemoryPixels(pixels, width, height, TEXTURE_TYPE_RGBA32)
     io.Fonts->TexID = (void*)textures_;
     ...
     while (true)
     {
        ...
        io.MousePos = my_mouse_pos;             // set the mouse position_
        ...
        // Scene imgui, swap buffers
        // (You want to try calling EndFrame/Scene as late as you can, to be able to use imgui in your own game rendering code)
        ImGui::EndFrame();
        ImGui::Scene();
 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE:
    ...
    void void MyImGuiRenderFunction(ImDrawData* draw_data)
    {
       ...
       // TODO: Setup shader: vertex { float2 pos, float2 uv, u32 color }, fragment shader sample color from 1 textures_, multiply by vertex color.
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             {
                 // The textures_ for the draw call is specified by pcmd->TextureId.
                 // The vast majority of draw calls will use the imgui textures_ atlas, which value you have set yourself during initialization.
                 ...
                 // Scene 'pcmd->ElemCount/3' indexed triangles.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your graphics engine render function after ImGui::Scene(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to display.
 - 2018/02/07 (1.60) - reorganized context handling to be more explicit,
                       - YOU NOW NEED TO CALL ImGui::CreateContext() AT THE BEGINNING OF YOUR APP, AND CALL ImGui::DestroyContext() AT THE END.
                       - removed Shutdown() function, as DestroyContext() serve this purpose.
                       - you may pass a ImFontAtlas* pointer to CreateContext() to share a font atlas between contexts. Otherwise CreateContext() will create its own font atlas instance_.
                       - removed allocator parameters from CreateContext(), they are now setup with SetAllocatorFunctions(), and shared by all contexts.
                       - removed the default global context and font atlas instance_, which were confusing for users of DLL reloading and users of multiple contexts.
 ...
 - 2015/08/29 (1.45) - with the addition of horizontal scrollbar we made various fixes to inconsistencies with dealing with cursor position_.
                       GetCursorPos()/SetCursorPos() functions now include the scrolled amount. It shouldn't affect the majority of users, but take note that SetCursorPosX(100.0f) puts you at +100 from the starting x position_ which may include scrolling, not at +100 from the window left side.
 ...
 - 2015/07/02 (1.42) - renamed GetScrollPosY() to GetScrollY(). Necessary to reduce confusion along with other scrolling functions, because positions (e.g. cursor position_) are not equivalent to scrolling amount.
 - 2015/06/14 (1.41) - changed ImageButton() default bg_col parameter from (0,0,0,1) (black) to (0,0,0,0) (transparent) - makes a difference when textures_ have transparence
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       font init:  { const void* png_data; unsigned int png_size; ImGui::GetDefaultFontData(NULL, NULL, &png_data, &png_size); <..Upload textures_ to GPU..>; }
                       became:     { unsigned char* pixels; int width, height; io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height); <..Upload textures_ to GPU>; io.Fonts->TexId = YourTextureIdentifier; }
                       you now have more flexibility to load multiple TTF fonts and manage the textures_ buffer for internal needs.
                       it is now recommended that you sample the font textures_ with bilinear interpolation.
              (1.30) - added textures_ identifier in ImDrawCmd passed to your render function (we can now render images). make sure to set io.Fonts->TexID.
 ...
 A: Short explanation:
    ...
    - Loading image files from the disk and turning them into a textures_ is not within the scope of Dear ImGui (for a good reason).
    ...
    - Each rendering function decides on a data type to represent "textures". The concept of what is a "textures_" is entirely tied to your underlying engine/graphics API.
      We carry the information to identify a "textures_" in the ImTextureID type.
    ...
    - In the examples/ bindings, for each graphics API binding we decided on a type that is likely to be a good representation for specifying
      ...
      For example, in the OpenGL example binding we store raw OpenGL textures_ identifier (GLuint) inside ImTextureID.
      ...
      tying together both the textures_ and information about its format and how to read it.
    - If you have a custom engine built over e.g. OpenGL, instead of passing GLuint around you may decide to use a high-level data type to carry information about
      the textures_ as well as how to display it (shaders, etc.). The decision of what to use as ImTextureID can always be made better knowing how your codebase
    ...
    User code may do:
        ...
        // Cast our textures_ type to ImTextureID / void*
        MyTexture* textures_ = g_CoffeeTableTexture;
        ImGui::Image((void*)textures_, ImVec2(textures_->Width, textures_->Height));

    The renderer function called after ImGui::Scene() will receive that same value that the user code passed:

        // Cast ImTextureID / void* stored in the draw command as our textures_ type
        MyTexture* textures_ = (MyTexture*)pcmd->TextureId;
        MyEngineBindTexture2D(textures_);
    ...
    Here's a simplified OpenGL example using stb_image.h:
        ...
        // Turn the RGBA pixel data into an OpenGL textures_:
        ...
        // Now that we have an OpenGL textures_, assuming our imgui rendering function (imgui_impl_xxx.cpp file) takes GLuint as ImTextureID, we can display it:
 ...
 A: Use the font atlas to pack them into a single textures_:
 ...
 A: When loading a font, pass custom Unicode ranges to specify the glyphs to load.
    ...
    the default implementation of io.ImeSetInputScreenPosFn() to set your Microsoft IME position_ correctly.
 ...
 A: - Being highly portable (bindings for several languages, frameworks, programming style, obscure or older platforms/compilers),
    ...
    - If you are finding your UI traversal cost to be too large, make sure your string usage is not leading to excessive amount
      ...
      This is a small helper where you can instance_ strings with configurable local buffers length. Many game engines will
 ...
 A: - You can create a dummy window. Call Begin() with the NoBackground | NoDecoration | NoSavedSettings | NoInputs flags.
    ...
    - You can create your own ImDrawList instance_. You'll need to initialize them ImGui::GetDrawListSharedData(), or create
 ...
 A: In your Scene function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f).
 ...
 - tip: you can call Scene() multiple times (e.g for VR renders).
...
// Debug options
#define IMGUI_DEBUG_NAV_SCORING     0   // Display navigation scoring preview when hovering items. Display last moving direction_ matches when holding CTRL
...
#pragma clang diagnostic ignored "-Wformat-pedantic"        // warning : format specifies type 'void *' but the argument has type 'xxxx *' // unreasonable, would lead to casting every %p_ arg to void*. probably enabled by -pedantic.
...
#pragma GCC diagnostic ignored "-Wformat"                   // warning: format '%p_' expects argument of type 'void*', but argument 6 has type 'ImGuiWindow*'
...
ImGuiStyle::ImGuiStyle()
{
    ...
    TouchExtraPadding       = ImVec2(0,0);      // Expand reactive bounding box for touch-based system where touch position_ is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    ...
    DisplayWindowPadding    = ImVec2(19,19);    // Window position_ are clamped to be visible within the display area by at least this amount. Only applies to regular windows.
...
// Trim str by offsetting contents when there's leading data + writing a \0 at the trailing position_. We use this in situation where the cost is negligible.
...
{
    // Set cursor position_ and a few other things so that SetScrollHereY() and Columns() can work when seeking cursor.
    ...
    if (window->DC.ColumnsSet)
        window->DC.ColumnsSet->LineMinY = window->DC.CursorPos.y;           // Setting this so that cell Y position_ are set properly
}
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of elements to display, and position_ the cursor before the first element.
    {
    ...
    if (StepNo == 3) // Step 3: the clipper validate that we have reached the expected Y position_ (corresponding to element DisplayEnd), advance the cursor to the end of the list and then returns 'false' to end the loop.
        End();
    return false;
}
...
// Scene a rectangle shaped with optional rounding and borders
...
// Scene an arrow aimed to be aligned with text (p_min is a position_ in the same space text would be positioned). To e.g. denote expanded/collapsed state
...
// Note that we still point to some static data and members (such as GFontAtlas), so the state instance_ you end up using will point to the static data within its module
...
// Same value as passed to the old io.RenderDrawListsFn function. Valid after Scene() and until the next call to NewFrame()
...
static void ImGui::UpdateMouseInputs()
{
    ...
    // Round mouse position_ to avoid spreading non-rounded position_ (e.g. UpdateManualResize doesn't support them well)
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        {
            // Maintain the maximum distance we reaching from the initial click position_, which is used with dragging threshold
            ImVec2 delta_from_click_pos = IsMousePosValid(&g.IO.MousePos) ? (g.IO.MousePos - g.IO.MouseClickedPos[i]) : ImVec2(0.0f, 0.0f);
...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
    ...
    // Tick io.WantCaptureMouse for the user application (true = dispatch mouse info to imgui, false = dispatch mouse info to imgui + app)
    if (g.WantCaptureMouseNextFrame != -1)
        g.IO.WantCaptureMouse = (g.WantCaptureMouseNextFrame != 0);
    else
        g.IO.WantCaptureMouse = (mouse_avail_to_imgui && (g.HoveredWindow != NULL || mouse_any_down)) || (!g.OpenPopupStack.empty());

    // Tick io.WantCaptureKeyboard for the user application (true = dispatch keyboard info to imgui, false = dispatch keyboard info to imgui + app)
    ...
    // Tick io.WantTextInput flag, this is to allow systems without a keyboard (e.g. mobile, hand-held) to show a software keyboard if possible
...
void ImGui::NewFrame()
{
    ...
    // Tick keyboard input state
    memcpy(g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration, sizeof(g.IO.KeysDownDuration));
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++)
        g.IO.KeysDownDuration[i] = g.IO.KeysDown[i] ? (g.IO.KeysDownDuration[i] < 0.0f ? 0.0f : g.IO.KeysDownDuration[i] + g.IO.DeltaTime) : -1.0f;

    // Tick gamepad/keyboard directional navigation
    NavUpdate();

    // Tick mouse input state
    UpdateMouseInputs();
    ...
    // But in order to allow the user to call NewFrame() multiple times without calling Scene(), we are doing an explicit clear.
...
// This is normally called by Scene(). You may want to call it directly if you want to avoid calling Scene() but the gain will be very minimal.
void ImGui::EndFrame()
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
...
void ImGui::Render()
{
    ...
    // (Legacy) Call the Scene callback function. The current prefer way is to let the user retrieve GetDrawData() and call the render function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
void ImGui::CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end)
{
    ...
    // When performing a navigation request, ensure we have one item extra in the direction_ we are moving to
...
// Find window given position_, search front-to-back
...
// We typically use ImVec2(-FLT_MAX,-FLT_MAX) to denote an invalid mouse position_.
...
// Return the delta from the initial clicking position_ while the mouse button is clicked or was just released.
// This is locked and return 0.0f until the mouse moves past a distance threshold at least once.
// NB: This is only valid if IsMousePosValid(). Back-ends in theory should always keep mouse position_ valid when dragging even outside the client window.
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    // Default/arbitrary window position_. Use SetNextWindowPos() with the appropriate condition flag to change the initial position_ of a window.
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        else if (held)
        {
            ...
            // We don't use an incremental MouseDelta but rather compute an absolute target size based on mouse position_
    ...
    // Apply back modified position_/size to window
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.FrameScopeActive);                  // Forgot to call ImGui::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::Scene() or ImGui::EndFrame() and haven't called ImGui::NewFrame() again yet
    ...
    // Tick Flags, LastFrameActive, BeginOrderXXX fields
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Tick stored window name when it changes (which can _only_ happen with the "###" operator, so the ID would stay unchanged).
        ...
        // Tick contents size from last frame for auto-fitting (or use explicit size)
        ...
        // Tick scrollbar status (based on the Size that was effective during last frame or the auto-resized Size).
        if (!window->Collapsed)
        {
        ...
        // Popup latch its initial position_, will position_ itself when it appears next frame
        ...
        // Clamp position_ so it stays visible
        ...
        else
        {
            ...
            // Scene resize grips (after their input handling so we don't have a frame of latency)
        ...
        // Tick various regions. Variables they depends on are set above in this function.
...
{
    window->DC.CursorMaxPos.x += window->Scroll.x; // SizeContents is generally computed based on CursorMaxPos which is affected by scroll position_, so we need to apply our change to it.
...
{
    window->DC.CursorMaxPos.y += window->Scroll.y; // SizeContents is generally computed based on CursorMaxPos which is affected by scroll position_, so we need to apply our change to it.
...
{
    // We store a target position_ so centering can occur on the next frame when we are guaranteed to have a known window size
    ImGuiWindow* window = GetCurrentWindow();
...
// Lock horizontal starting position_ + capture group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
...
//      pos_x != 0      : align to specified x position_ (relative to window/group left)
...
void ImGui::BeginTooltip()
{
    ...
    {
        // The default tooltip position_ is a little offset to give space to see the context menu (it's also clamped within the current viewport/monitor)
        // In the context of a dragging tooltip we try to reduce that offset and we enforce following the cursor.
        // Whatever we do we want to call SetNextWindowPos() to enforce a tooltip position_ and disable clipping the tooltip without our display area, like regular tooltip do.
...
ImVec2 ImGui::FindBestWindowPosForPopupEx(const ImVec2& ref_pos, const ImVec2& size, ImGuiDir* last_dir, const ImRect& r_outer, const ImRect& r_avoid, ImGuiPopupPositionPolicy policy)
{
    ...
    if (policy == ImGuiPopupPositionPolicy_ComboBox)
    {
        ...
        for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
        {
            ...
            if (n != -1 && dir == *last_dir) // Already tried this direction_?
                continue;
            ImVec2 pos;
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    {
        // Child menus typically request _any_ position_ within the parent menu item, and then we move the new menu outside the parent bounds.
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    if (quadrant == g.NavMoveDir)
    {
        ...
        if (dist_box == result->DistBox)
        {
            ...
            else if (dist_center == result->DistCenter)
            {
                ...
                // this is fairly easy. This rule ensures that all buttons with dx==dy==0 will end up being linked in order of appearance_ along the x axis.
    ...
    // Axial check: if 'curr' has no link at all in some direction_ and 'cand' lies roughly in that direction_, add a tentative link. This will only be kept if no "real" matches
    // are found, so it only augments the graph produced by the above method using extra links. (important, since it doesn't guarantee strong connectedness)
    // This is just to avoid buttons having no links in a particular direction_ when there's a suitable neighbor. you get good graphs without this too.
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb, const ImGuiID id)
{
    ...
    // Tick window-relative bounding box of navigated item
    if (g.NavId == id)
    {
        ...
        window->NavRectRel[window->DC.NavLayerCurrent] = nav_bb_rel;    // Store item bounding box (relative to window position_)
    }
}
...
static ImVec2 ImGui::NavCalcPreferredRefPos()
{
    ...
    {
        // When navigation is active and mouse is disabled, decide on an arbitrary position_ around the bottom left of the currently navigated item.
        const ImRect& rect_rel = g.NavWindow->NavRectRel[g.NavLayer];
        ImVec2 pos = g.NavWindow->Pos + ImVec2(rect_rel.Min.x + ImMin(g.Style.FramePadding.x * 4, rect_rel.GetWidth()), rect_rel.Max.y - ImMin(g.Style.FramePadding.y, rect_rel.GetHeight()));
        ImRect visible_rect = GetViewportRect();
        return ImFloor(ImClamp(pos, visible_rect.Min, visible_rect.Max));   // ImFloor() is important because non-integer mouse position_ application in back-end might be lossy and result in undesirable non-zero delta.
    }
}
...
static void ImGui::NavUpdate()
{
    ...
    // Tick Keyboard->Nav inputs mapping
    ...
    // Apply application mouse position_ movement, after we had a chance to process move request result.
    if (g.NavMousePosDirty && g.NavIdIsAlive)
    {
        // Set mouse position_ given our knowledge of the navigated item position_ from last frame
    ...
    // Tick CTRL+TAB and Windowing features (hold Square to move/resize/etc.)
    NavUpdateWindowing();
    ...
    // Tick PageUp/PageDown scroll
    float nav_scoring_rect_offset_y = 0.0f;
    if (nav_keyboard_active)
        nav_scoring_rect_offset_y = NavUpdatePageUpPageDown(allowed_dir_flags);

    // If we initiate a movement request and have no current NavId, we initiate a InitDefautRequest that will be used as a fallback if the direction_ fails to find a match
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    if (g.NavLayer == 0)
    {
        ...
        // Estimate upcoming scroll so we can offset our result position_ so mouse position_ can be applied immediately after in NavUpdate()
        ...
        if (page_up_held != page_down_held) // If either (not both) are pressed
        {
            ...
            else
            {
                ...
                if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
                {
                    ...
                    g.NavMoveDir = ImGuiDir_Down; // Because our scoring rect is offset, we intentionally request the opposite direction_ (so we can always land on the last item)
                ...
                {
                    nav_scoring_rect_offset_y = +page_offset_y;
                    g.NavMoveDir = ImGuiDir_Up; // Because our scoring rect is offset, we intentionally request the opposite direction_ (so we can always land on the last item)
...
// Internal version that takes a position_ to decide on newline placement and pad items according to their depth.
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    for (;;)
    {
        ...
        {
            // An empty "" string at a different Y position_ should output a carriage return.
            LogText(IM_NEWLINE);
            break;
        }
...
static void ImeSetInputScreenPosFn_DefaultImpl(int x, int y)
{
    // Notify OS Input Method Editor of text input position_
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            ImDrawList* fg_draw_list = GetForegroundDrawList(window); // Scene additional visuals into the top-most draw list
